...
class Cross_Entropy_np():
    ...
    def forward(self, pred:np.array, target:np.array, ignore_idx:int=0 ):
        """
    ...
    def backward(self, d_prev=1):
        ...
        ignore_matrix = np.where(self.target_origin==self.ignore_idx, 0.01, 1).reshape(1,-1,1) #it works only dim=3 !
        
        grad = grad * ignore_matrix
